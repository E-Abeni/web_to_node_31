...
export default function Reports () {
  ...
  const selectedCustomer = selectedSTR
    ...
    return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 p-10 min-h-[100vh] bg-[#09141f]">
      {/* List of STRs */}
      <div className="lg:col-span-1 bg-slate-900 border border-slate-800 rounded-lg flex flex-col overflow-hidden shadow-sm">
          ...
          <div className="flex gap-2">
            <div className="relative flex-1 group">
              ...
              <input 
                ...
                className="w-full bg-slate-950 border border-slate-700 rounded pl-7 pr-8 py-1.5 text-xs focus:outline-none focus:border-blue-500 text-white placeholder:text-slate-300 transition-all"
                value={searchText}
            ...
            <div className="relative w-36">
              ...
              <select 
                  className="w-full bg-slate-950 border border-slate-700 rounded pl-7 pr-2 py-1.5 text-xs focus:outline-none focus:border-blue-500 appearance-none text-slate-300"
                  value={filterStatus}
          ...
          {filteredSTRs.length === 0 ? (
            ...
            filteredSTRs.map((str) => (
              <div 
                ...
                className={`p-4 border-b border-slate-800 cursor-pointer transition-colors ${selectedSTR?.id === str.id ? 'bg-blue-900/20 border-l-4 border-l-blue-500' : 'hover:bg-slate-800/50 border-l-4 border-l-transparent'}`}
              >
                <div className="flex justify-between items-start mb-1">
                  <span className="text-xs font-mono text-slate-500">{str.id}</span>
                  <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${str.status === 'PENDING' ? 'bg-yellow-900/30 text-yellow-500' : str.status === 'DISMISSED' ? 'bg-red-900/30 text-red-500' : 'bg-green-900/30 text-green-500'}`}>
                    {str.status}
                  </span>
                </div>
                <div className="text-sm font-medium text-slate-200 mb-1">{getDisplayReason(str)}</div>
                <div className="text-xs text-slate-400">{str.generatedAt.toLocaleTimeString()}</div>
              </div>
            ))
          )}
        </div>
      ...
      {/* STR Detail & Analysis */}
      <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-lg flex flex-col p-6 overflow-auto shadow-sm">
        {selectedSTR ? (
          <div className="space-y-6">
            <div className="flex justify-between items-start">
              <div>
                <h3 className="text-xl font-bold text-white mb-2">Investigation Details</h3>
                <p className="text-slate-400 text-sm">Case Reference: <span className="font-mono text-slate-300">{selectedSTR.id}</span></p>
              </div>
              <div className="flex gap-2">
                <button className="flex items-center gap-2 px-3 py-1.5 rounded bg-green-900/30 text-green-400 border border-green-800 hover:bg-green-900/50 text-sm transition-colors">
                  <CheckCircle className="w-4 h-4" /> File SAR
                </button>
                <button className="flex items-center gap-2 px-3 py-1.5 rounded bg-red-900/30 text-red-400 border border-red-800 hover:bg-red-900/50 text-sm transition-colors">
            ...
            <div className="bg-slate-950 p-4 rounded border border-slate-800">
               ...
               <p className="text-slate-400 text-sm leading-relaxed">{getDisplayReason(selectedSTR)}</p>
            </div>
              ...
              <div className="border-t border-slate-800 pt-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-slate-950 p-4 rounded border border-slate-800">
                    ...
                    <div className="mt-1 font-mono text-sm text-blue-400 break-all">{selectedTransaction.id}</div>
                  </div>
                  <div className="bg-slate-950 p-4 rounded border border-slate-800">
                    <span className="text-xs text-slate-500 uppercase tracking-wider font-semibold">Amount</span>
                    <div className="mt-1 font-mono text-lg text-white">
                      {selectedTransaction.amount.toLocaleString(undefined, { minimumFractionDigits: 2 })} <span className="text-sm text-slate-400">{selectedTransaction.currency}</span>
                    </div>
                  </div>
                  <div className="bg-slate-950 p-4 rounded border border-slate-800">
                  ...
                  <div className="bg-slate-950 p-4 rounded border border-slate-800">
                    ...
                    <div className="mt-1 text-sm text-slate-200">{selectedTransaction.destinationCountry}</div>
                  </div>
                    ...
                    <div className="mt-1 flex flex-wrap gap-2">
                      {selectedTransaction.flags.length > 0 ? (
                        selectedTransaction.flags.map((flag, idx) => (
                          <span key={idx} className="px-2 py-1 rounded text-xs font-medium bg-red-900/30 text-red-400 border border-red-800">
                            {flag}
                          </span>
                        ))
                      ) : (
              ...
              <div className="border-t border-slate-800 pt-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-slate-950 p-4 rounded border border-slate-800">
                    ...
                    <div className="mt-1 font-medium text-slate-200">{selectedCustomer.name}</div>
                  ...
                  <div className="bg-slate-950 p-4 rounded border border-slate-800">
                    ...
                    <div className="mt-1">
                       <span className={`px-2 py-1 rounded-full text-xs font-bold border ${
                          selectedCustomer.riskLevel === 'CRITICAL' ? 'bg-red-900/30 text-red-400 border-red-800' :
                          selectedCustomer.riskLevel === 'HIGH' ? 'bg-orange-900/30 text-orange-400 border-orange-800' :
                          selectedCustomer.riskLevel === 'MEDIUM' ? 'bg-yellow-900/30 text-yellow-400 border-yellow-800' :
                          'bg-green-900/30 text-green-400 border-green-800'
                       }`}>
                        {selectedCustomer.riskLevel}
                      </span>
                      <span className="ml-2 text-xs text-slate-400">Score: {selectedCustomer.riskScore}</span>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-950 p-4 rounded border border-slate-800">
                    ...
                    <div className="mt-1 text-sm text-slate-200">{selectedCustomer.occupation}</div>
                  </div>
                  ...
                  <div className="bg-slate-950 p-4 rounded border border-slate-800">
                    ...
                    <div className="mt-1 text-sm text-slate-200">{selectedCustomer.region}</div>
                  </div>
                      ...
                      {selectedCustomer.tags.length > 0 || selectedCustomer.isSanctioned ? (
                        <>
                          {selectedCustomer.isSanctioned && (
                             <span className="px-2 py-1 rounded text-xs font-bold bg-red-950 text-red-500 border border-red-900 animate-pulse">
                               SANCTION MATCH
                             </span>
                          )}
                          {selectedCustomer.tags.map((tag, idx) => (
                            <span key={idx} className="px-2 py-1 rounded text-xs font-medium bg-indigo-900/30 text-indigo-400 border border-indigo-800">
                              {tag}
                            </span>
                          ))}
                        </>
